...
namespace ItegrationalTests.IntegrationalTests
{
    ...
    public class UserIntegrationTests
    {
        ...
        public void TestRegister()
        {
            ...
            var user = userOM.CreateReferee().WithName("testname").WithLogin("testlogin123").WithEmail("aa@mail.ru").WithConfirmedEmail().BuildCoreModel();
            var cnt = _fixture._dbContextFactory.get_db_context().users.Count();

            _userService.Register(user.Login, user.Password, user.Role, user.Name, user.Email);
        ...
        public void TestChangeInfo()
        {
            Skip.If(IntegrationFixture.SkipTest);
            var users = _fixture.AddUsers();
            var u = userOM.CreateReferee().WithId(users.First().Id).WithName("mytest").WithEmail("aa@mail.ru").WithConfirmedEmail().WithLogin("test123").BuildCoreModel();
