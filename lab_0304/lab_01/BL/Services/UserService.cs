...

{
    public class UserService
    {
        ...
        public void Register(string login, string password, string role, string name, string email, bool isConfirmed = false)
        {
            logger.LogInformation("register started");
            if (userRepo.readByLogin(login) == null)
            {
                userRepo.create(new User(login, password, role, name, email, isConfirmed));
                logger.LogInformation("register success");
            }
            else
            {
        ...
        {
            logger.LogInformation("confirm email started");
            User? u = userRepo.readById(id);
            if (u != null)
            {
                u.IsEmailConfirmed = true;
                userRepo.update(u);
                logger.LogInformation("confirm email ended");
            }
            else
            {
                logger.LogError("read user by id failed");
        ...
        {
            return userRepo.readByLogin(login).Id;
        }
    }
}
