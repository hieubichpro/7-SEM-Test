...
namespace E2ETests
{
    ...
    public class E2ETests
    {
        ...
        //private 
        private dbContextFactory _dbContextFactory = new pgSqlDbContextFactory("Server=postgres;Username=postgres;Password=123456789;Database=TestTestTest");
        ...
        public void TestReferee()
        {
            ...
            var user = userOM.CreateReferee().WithLogin("teste2e").WithPassword("teste2e").WithPassword("teste2e").WithEmail("aa@mail.ru").WithConfirmedEmail().BuildCoreModel();
            int cnt = _userRepo.readAll().Count;

            _userService.Register(user.Login, user.Password, user.Role, user.Name, user.Email, user.IsEmailConfirmed);
